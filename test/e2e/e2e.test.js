...
describe('Bunosh End-to-End Tests', () => {
  ...
  describe('Bunosh Initialization', () => {
    ...
    it('should show help without Bunoshfile', async () => {
      const result = await runBunoshCommand('--help', { cwd: testDir });
      
      expect(result.success).toBe(true);
      expect(result.stdout).toContain('bunosh');
      expect(result.stdout).toContain('Commands are loaded from');
    });
  });
  ...
  describe('Task Execution', () => {
    ...
    it('should execute yell task with cfonts output', async () => {
      const result = await runBunoshCommand('yell:task TESTING', { cwd: testDir });
      
      expect(result.success).toBe(true);
      // cfonts produces ASCII art, so just check it ran successfully
      expect(result.exitCode).toBe(0);
    });
  ...
  describe('Command Listing and Help', () => {
    ...
    it('should show mixed bunosh and npm commands in alphabetical order', async () => {
      ...
      const commands = commandSection
        ...
        .filter(cmd => cmd && !cmd.startsWith('bunosh'));
      ...
      expect(commands.some(cmd => cmd.includes('simple:exec'))).toBe(true);
      expect(commands.some(cmd => cmd.includes('npm:'))).toBe(true);
    });
